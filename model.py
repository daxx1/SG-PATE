...
class DCGAN(object):
    def __init__(self, sess, input_height=32, input_width=32, crop=False,
        ...
        self.grad_L1=[]
        self.grad_cliplist=[]
        self.grad_fincliplist = []
        self.grad_printclip = []
        self.grad_aux = [] #辅助寻找裁剪值对应的L2范数值
        self.grad_auxclip = []
    ...
    def aggregate_results(self, output_list, config, thresh=None, epoch=None,check_epoch=None):
        if self.pca and check_epoch<40:
            print('use---------pca')
    ...
    def load_fashion_mnist(self):
        ...
        #data_dir = r"D:\DataLens\data\fashion_mnist"
    ...
    def train_together(self, config):
        ...
        if self.pca:
            ...
            data_x, data_y = self.load_fashion_mnist()
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                for k in range(config.g_step):
                    ...
                    self.grad_printclip.append(self.step_size)
                    np.savetxt(self.checkpoint_dir + "/grad_printclip.txt", np.asarray(self.grad_printclip), delimiter=",")
                    for j in range(self.batch_size):
                        ...
                        if config.non_private:
                            ...
                            img_grads_agg_tmp, rdp_budget = self.aggregate_results(
                                [grads[j] for grads in img_grads_list], config, thresh=thresh, check_epoch=epoch)
                    ...
                    for l in range(self.batch_size):
                        res_clip = res_clip + np.linalg.norm(np.asarray(img_grads_agg_list)[l].flatten(), ord=2)
                        res_L1 = res_L1 + np.linalg.norm(np.asarray(img_grads_agg_list)[l].flatten(), ord=1)
                    self.grad_L2.append(res_clip/self.batch_size)
                    self.grad_L1.append(res_L1/self.batch_size)
                    np.savetxt(self.checkpoint_dir + "/dp_epsgradL2.txt", np.asarray(self.grad_L2),delimiter=",")
                    np.savetxt(self.checkpoint_dir + "/dp_epsgradL1.txt", np.asarray(self.grad_L1),delimiter=",")
                    '''
                    res_clip = 0
                    for l in range(self.batch_size):
                        res_clip = res_clip + np.linalg.norm(np.asarray(img_grads_agg_list)[l].flatten(), ord=1)
                    self.grad_cliplist.append(res_clip/self.batch_size)
                    self.grad_aux.append(res_clip/self.batch_size)
                    np.savetxt(self.checkpoint_dir + "/grad_aux.txt", np.asarray(self.grad_aux), delimiter=",")
                    if epoch == 0:
                        self.grad_fincliplist.append(self.step_size)
                    if epoch != 0:
                        
                        if  epoch < 19:
                            list_index = len(self.grad_cliplist) - 1
                            list_vals = 1.0 + (self.grad_cliplist[list_index] - self.grad_cliplist[0])/self.grad_cliplist[0]
                            self.grad_fincliplist.append(list_vals*self.step_size)
                            ...
                            if epoch ==40:
                                ...
                                list_index = len(self.grad_cliplist) - 1
                                list_vals = 1.0 + (self.grad_cliplist[list_index] - self.grad_cliplist[list_index-1])/self.grad_cliplist[list_index-1]
                                self.grad_fincliplist.append(list_vals*self.step_size)
                    
                    np.savetxt(self.checkpoint_dir + "/dp_epsgrad.txt", np.asarray(self.grad_cliplist),delimiter=",")
                    np.savetxt(self.checkpoint_dir + "/dp_epsgradreal.txt", np.asarray(self.grad_fincliplist), delimiter=",")
                    ...
                    if not self.save_dict[floor_eps]:
                        ...
                        data = data[:100000]
                        import joblib
                        joblib.dump(data, self.checkpoint_dir + '/eps-%.2f.data' % floor_eps)
